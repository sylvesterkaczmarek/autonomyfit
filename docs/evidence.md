...
AutonomyFit separates model metadata from deployment evidence. A model name is not a benchmark identity.
...
Reports and direct Python benchmark evidence reject NaN and infinity before scoring. Missing measurements remain unknown; a reciprocal median latency is never substituted for measured throughput.
...
These rows do not publish throughput, so their FPS field is unknown. A batch execution, a single-item inference and an asynchronous throughput measurement are distinct quantities.

## Power and memory scope

Power needs an explicit scope before it can satisfy a constraint or influence power ranking. Jetson VDD_IN rail readings and NVIDIA GPU `power.draw` are not treated as whole-system wall power. TensorRT whole-command telemetry is retained for diagnostics and excluded from inference-power constraint evidence. MLPerf power measurements use their own standardized system-level methodology.

The generic local memory sampler reports process RSS. `--max-memory-gb` instead applies to the model's published or estimated deployment-memory screen. Passing that screen, including within a `VERIFIED_FIT` recommendation, does not establish measured accelerator-memory consumption.
